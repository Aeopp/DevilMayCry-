...
void MapTool::ShowPivotOption()
{
	...
	Vector3 vPivotPos = m_pPivot.lock()->Get_Trans().lock()->GetPosition();  // pivot pos Get 
...
void MapTool::PeekingOptionGroup()
{
	if (ImGui::CollapsingHeader("==============PeekingOption=============", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		{
			if (m_strSelectFBXName.empty()) // select file 이 안 되어 있는 경우 
			{
				ImGui::TextColored(ImVec4(1, 0, 0, 1), "Select FBX Name ");
				m_eCreateOption = eCreatePosition::End;
			}
			else  // 선택한 파일 이름 과 아이디 그리고 생성 위치를 선택할 수 있게 함 
			{
				ImGui::TextColored(ImVec4(1, 0, 0, 1), "Select Props : %s \tID: %d", convertToString(m_strSelectFBXName).c_str(), m_iTableID);
			...
			{
				if (m_pBaseMap.expired()) // 베이스 맵이 로딩이 안될 경우 
				{
					m_bCreateLock = true; // 생성 락  후 경고 텍스트 
					ImGui::TextColored(ImVec4(1, 0, 0, 1), "Not Exist Base Map"); 
				}
				else
					m_bCreateLock = false;
			}
			else // 피봇위치는 상관없으니 락 품 
				m_bCreateLock = false;
		}

		/// Peeking Type
		if (m_eWorkType == eWorkOption::Modify) //수정 모드 일때만 피킹 타입 선택 가능함 
		{
			ImGui::Text("PeekingType");
			ImGui::RadioButton("Single", (int*)&m_ePeekType, (int)ePeekingType::Single);ImGui::SameLine();
			...
			{
				ClearMultiPeeking();//싱글타입으로 교체시 선택한 오브젝트 이름 표시 
				ImGui::Text("Peeking Object Name : "); ImGui::SameLine();
				ImGui::TextColored(ImVec4(1, 0, 0, 1), "%s", m_strPeekingName.empty() ? "None Peeking" : convertToString(m_strPeekingName).c_str());
			}
			else //멀티 피킹시 
			{
				//현재 피킹 구현이 안되있음으로 막음 
...
void MapTool::TransFormCtrlGroup()
{
	...
	if (ImGui::CollapsingHeader("==============TransForm=============", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		if (m_ePeekType == ePeekingType::Single)
		{
			...
			Vector3 vRot = m_pCurSelectObj.lock()->Get_Trans().lock()->GetRotation(); // obj pos Get 
			Vector3 vPos = m_pCurSelectObj.lock()->Get_Trans().lock()->GetPosition();  // obj pos Get 
		...
		else
		{
			...
			float fScale = 0.f;
			Vector3 vRot = vZero;// = vector(0,0,0)
			Vector3 vPos = vZero;
			...
			//{
			//	//Trnasform add 함수 필요 
			//	pObj.lock()->Get_Trans().lock()->AddSacle()
			//	pObj.lock()->Get_Trans().lock()->AddSRot()
			//	pObj.lock()->Get_Trans().lock()->AddPosition()
			//}
		}
	}
}
